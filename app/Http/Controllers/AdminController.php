 ...
 public function users(Request $request)
    {
        ...
        // 🔑 Updated ordering: show most recently updated users (balance activity) on top
        $users = $usersQuery
            ->orderBy('updated_at', 'desc') // activity first
            ->orderBy('created_at', 'desc') // fallback
            ->get(['id', 'name', 'mobile_number', 'invitation_code', 'balance', 'frozen_balance', 'created_at', 'updated_at'])
            ->map(function ($user) {
...
public function updateUser(Request $request, $id)
 ...
 public function withdraw()
    ...
    //     // also update order here to reflect recent activity
    //     $users = $usersQuery
    ...
    //         'coupon_threshold' => 'required|integer|min:0',
    //     ]);

    //     $user = User::findOrFail($request->user_id);
    //     $products = $request->products;

    //     Log::info('Order sent to user ID: ' . $user->id, ['products' => $products]);

    //     return redirect()->back()->with('success', 'Order sent successfully to user: ' . $user->name);
    // }
    ...
    public function updateWallet(Request $request)
    {
        ...
        if ($userId) {
            $user = User::select('id', 'name', 'mobile_number', 'balance', 'frozen_balance')
                ->find($userId);
            if ($user) {
                $user->balance = (float) ($user->balance ?? 0.00);
                $user->frozen_balance = (float) ($user->frozen_balance ?? 0.00);
...
{
    $luckyProduct = \App\Models\Product::where('type', 'Lucky Order')->inRandomOrder()->first();
    if ($luckyProduct) {
...
public function resetUserTasks(User $user)
{
    ...
    \App\Models\UserOrder::where('user_id', $user->id)->delete();
    return response()->json(['success' => true]);
}
public function deleteUserTasks(User $user)
{
    \App\Models\Task::where('user_id', $user->id)->delete();
    return response()->json(['success' => true]);
}
