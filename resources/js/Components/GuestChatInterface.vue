            ...
            <div v-else v-for="message in messages" :key="message.id" 
                ...
                <div class="flex items-start space-x-2 max-w-[85%]"
                    ...
                    <div class="rounded-lg p-3 text-sm bg-gradient-to-br from-cyan-400/20 via-blue-500/15 to-indigo-600/20 backdrop-blur-xl border border-cyan-300/30"
                        ...
                        <div v-if="message.message" class="text-sm text-white whitespace-pre-wrap">{{ message.message }}</div>
                    </div>
                </div>
            </div>
        </div>
...
const handleImageUpload = async (event) => {
    try {
        ...
        if (tempIndex > -1) {
            ...
            if (tempMsg.image_path?.startsWith('blob:')) {
                URL.revokeObjectURL(tempMsg.image_path);
            }
...
const handleVideoUpload = async (event) => {
    try {
        ...
        if (tempIndex > -1) {
            const tempMsg = messages.value[tempIndex];
            if (tempMsg.video_path?.startsWith('blob:')) {
                URL.revokeObjectURL(tempMsg.video_path);
            }
            messages.value.splice(tempIndex, 1);
        }
    }
